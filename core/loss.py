...
def batch_episym(x1, x2, F):
    ...
    x2Fx1 = torch.matmul(x2.transpose(2,3).contiguous(), torch.matmul(F, x1)).reshape(batch_size,num_pts)
    Fx1 = torch.matmul(F,x1).reshape(batch_size,num_pts,3)
    Ftx2 = torch.matmul(F.transpose(2,3).contiguous(),x2).reshape(batch_size,num_pts,3)
    ys = x2Fx1**2 * (
...
class MatchLoss(object):
  ...
  def run(self, global_step, data, logits, e_hat):
    ...
    if self.use_fundamental:
        ess_hat = torch.matmul(torch.matmul(data['T2s'].transpose(1,2).contiguous(), ess_hat.reshape(-1,3,3)),data['T1s'])
        # get essential matrix from fundamental matrix
        ess_hat = torch.matmul(torch.matmul(data['K2s'].transpose(1,2).contiguous(), ess_hat.reshape(-1,3,3)),data['K1s']).reshape(-1,9)
