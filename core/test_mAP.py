...
def dump_res(measure_list, res_path, eval_res, tag):
    ...
    ret_val_5 = np.mean(qt_acc[:1])
    ret_val_10 = np.mean(qt_acc[:2])
    ret_val_20 = np.mean(qt_acc[:4])  # 1 == 5

    return [ret_val_5, ret_val_10, ret_val_20]
...
def test_process(mode, model, cur_global_step, data_loader, config):
    ...
    if config.res_path == '':
        config.res_path = os.path.join(config.log_path, mode)
...
def test_mAP(data_loader, model, config):
    ...
    if config.res_path == '':
        config.res_path = os.path.join(config.model_path, "test_mAP")
        if not os.path.isdir(config.res_path):
...
if __name__ == '__main__':
    model_config = torch.load(os.path.join(config.model_path, 'config.th'))
    ...
    test_mAP(test_loader, model, config)
